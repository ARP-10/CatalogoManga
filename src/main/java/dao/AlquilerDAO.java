...
public class AlquilerDAO extends BaseDAO<Alquiler> implements CrudDAO<Alquiler>{
    ...
    private static final String INSERT_ALQUILER = "INSERT INTO manga_db.alquileres (fecha_inicio, fecha_fin, estado) VALUES (?, ?, ?)";
    private static final String UPDATE_ALQUILER = "UPDATE manga_db.alquileres SET fecha_inicio = ?, fecha_fin = ?, estado = ? WHERE id = ?";
	...
	protected Alquiler mapResultSetToEntity(ResultSet rs) throws SQLException {
		return new Alquiler(rs.getInt("id"), rs.getDate("fecha_inicio"), rs.getDate("fecha_fin"), rs.getBoolean("estado"));
	}
	...
	public void crearNuevoCrud(Alquiler alquiler) throws SQLException {
		crearNuevo(INSERT_ALQUILER, alquiler.getFecha_inicio(), alquiler.getFecha_fin(), alquiler.isEstado());
		
	}

	@Override
	public void actualizarCrud(Alquiler alquiler) throws SQLException {
		actualizar(UPDATE_ALQUILER, alquiler.getFecha_inicio(), alquiler.getFecha_fin(), alquiler.isEstado());
